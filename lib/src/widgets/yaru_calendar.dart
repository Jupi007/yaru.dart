...
class _YaruDayPickerState extends State<YaruDayPicker> {
  ...
  void initState() {
    ...
    displayedMonth = widget.initialDate ?? now;
      ...
      children: [
        if (!yearSelectionMode)
          IconButton(
            onPressed: () => setState(() {
              displayedMonth =
                  DateUtils.addMonthsToMonthDate(displayedMonth, -1);
            }),
  ...
  Widget _buildMonthGrid() {
    return YaruMonthGrid(
      ...
      dayHeaderBuilder: (i) => _YaruDayHeader(
        index: i,
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: DecoratedBox(
        ...
        child: Center(
          child: Text(localizations.narrowWeekdays[index]),
        ),
      ),
    );
  }
...
class _YaruDayButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (selected) {
      color = colorScheme.onPrimary.withOpacity(currentMonth ? 1 : .5);
    } else {
      color = colorScheme.onSurface.withOpacity(currentMonth ? 1 : .5);
    }

    late final Color? background;
    if (selected) {
      background = colorScheme.primary.withOpacity(currentMonth ? 1 : .25);
    } else if (today) {
      background = colorScheme.onSurface.withOpacity(0.1);
    } else {
      background = null;
    }
...
class _YaruMonthPicker extends StatefulWidget {
  ...
  final DateTime firstDate;
  final DateTime lastDate;
  final void Function(DateTime year) onMonthSelected;
...
class _YaruMonthPickerState extends State<_YaruMonthPicker> {
  ...
  void initState() {
    ...
    controller = PageController(
      viewportFraction: .2,
      initialPage: selectedMonth.month - 1,
      keepPage: false,
    );
        ...
        return _YaruMonthYearPickerItem(
          ...
          child: Text(
            DateFormat.MMMM().format(
              selectedMonth.copyWith(month: index + 1),
            ),
          ),
        );
      },
...
class _YaruYearPicker extends StatefulWidget {
  ...
  final DateTime firstDate;
  final DateTime lastDate;
  final void Function(DateTime year) onYearSelected;
...
class _YaruYearPickerState extends State<_YaruYearPicker> {
  ...
  void initState() {
    ...
    controller = PageController(
      viewportFraction: .2,
      initialPage: selectedYear.year - widget.firstDate.year,
      keepPage: false,
    );
        ...
        return _YaruMonthYearPickerItem(
          ...
          onTap: () {
            ...
            controller.jumpToPage(
              selectedYear.year - widget.firstDate.year,
            );
          },
